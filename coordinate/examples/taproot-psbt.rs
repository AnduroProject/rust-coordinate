...
use coordinate::bip32::{ChildNumber, DerivationPath, Fingerprint, Xpriv, Xpub};
use coordinate::consensus::encode;
use coordinate::key::{TapTweak, XOnlyPublicKey};
use coordinate::opcodes::all::{OP_CHECKSIG, OP_CLTV, OP_DROP};
use coordinate::psbt::{self, Input, Output, Psbt, PsbtSighashType};
use coordinate::secp256k1::Secp256k1;
use coordinate::sighash::{self, SighashCache, TapSighash, TapSighashType};
use coordinate::taproot::{self, LeafVersion, TapLeafHash, TaprootBuilder, TaprootSpendInfo};
use coordinate::{
...
fn generate_bip86_key_spend_tx(
    ...
    let tx1 = Transaction {
        ...
        input: vec![TxIn {
            ...
            sequence: coordinate::Sequence(0xFFFFFFFF), // Ignore nSequence.
    ...
    fn create_inheritance_funding_tx(
        ...
        let next_tx = Transaction {
            ...
            input: vec![TxIn {
                ...
                script_sig: ScriptBuf::new(),
                sequence: coordinate::Sequence(0xFFFFFFFD), // enable locktime and opt-in RBF
                witness: Witness::default(),
            }],
            output: vec![],
        };
